...
for ($i=0; $i -lt 8; $i++) {
    $newMain += Get-CardHTML "card-glass" "Trending" $i "Best Seller"
}
...
# Correct Footer extraction - ensure it's clean
$footerTag = "<footer"
$footerStartPos = $content.IndexOf($footerTag)
$bottomNavTag = '<div class="mobile-bottom-nav">'
$bottomNavStartPos = $content.IndexOf($bottomNavTag)
...
Write-Host "Rebuilt featured.html with fixed click events and better pricing."
